...
class DiffBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.word_embeddings = nn.Linear(config.vocab_size, config.hidden_size) #one hot to hidden
...
class DiffBertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in DiffBertModel forward() function)
...
class DiffBertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = DiffBertSelfAttention(config, position_embedding_type=position_embedding_type)
